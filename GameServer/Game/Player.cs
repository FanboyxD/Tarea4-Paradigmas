...
namespace PlatformGameServer.Game
{
    public class Player
    {
        ...
        {
            Score += points;
            Console.WriteLine($"Player {PlayerId} obtuvo {points} puntos. Score total: {Score}");
        }
        ...
        private void CheckCollisions(int[,] gameMap)
        {
            ...
            // Verificar pared a la derecha
            if (playerTileX + 1 < mapWidth && gameMap[playerTileY, playerTileX + 1] == 1 && VelocityX > 0)
            {
                X = playerTileX;
                VelocityX = 0;
            }

            // Verificar pared a la izquierda
            if (playerTileX > 0 && gameMap[playerTileY, playerTileX - 1] == 1 && VelocityX < 0)
            {
                X = playerTileX;
                VelocityX = 0;
            }
