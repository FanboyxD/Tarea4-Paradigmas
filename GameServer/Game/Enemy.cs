...
namespace PlatformGameServer.Game
{
    ...
    public abstract class Enemy
    {
        ...
        public bool CanCauseDamage { get; set; } = true; // Propiedad para controlar si puede hacer daño
    ...
    // Abstract Factory para frutas (ya existente, pero renombrada para consistencia)
    ...
    public static class EnemyCreator
    {
        ...
        public static Enemy CreateEnemy(EnemyType enemyType, float x, float y)
        {
            var factory = EnemyFactoryProvider.GetFactory(enemyType);
            return factory.CreateEnemy(x, y);
        }

        // Crear GroundEnemy con dirección específica
        public static Enemy CreateGroundEnemy(float x, float y, bool fromLeft = true)
        {
            var factory = EnemyFactoryProvider.GetGroundEnemyFactory(fromLeft);
            return factory.CreateEnemy(x, y);
        }
        ...
        // Crear enemigo aleatorio (excluyendo frutas)
        public static Enemy CreateRandomEnemy(float x, float y, Random random = null)
        {
            random = random ?? new Random();
            var availableTypes = EnemyFactoryProvider.GetAvailableEnemyTypes().Where(t => t != EnemyType.Fruit).ToArray();
            var selectedType = availableTypes[random.Next(availableTypes.Length)];
            
            if (selectedType == EnemyType.Ground)
            {
                return CreateGroundEnemy(x, y, random.Next(0, 2) == 0);
            }
            
            return CreateEnemy(selectedType, x, y);
        }
