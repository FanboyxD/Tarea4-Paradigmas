...
public class NetworkManager {
    ...
    try {
        ...
        for (int i = 0; i < MAX_FRUITS; i++) {
            fruits[i] = new Fruit(0, 0, FruitType.NONE);
        }
        ...
        for (int i = 0; i < MAX_FRUITS && buffer.remaining() >= 16; i++) {
            ...
            if (active == 1 && !fruits[i].isActive()) {
                fruits[i].spawn(x * Client.getCellSize(), y * Client.getCellSize());
            }
            
            fruits[i].setGridPosition(x, y);
            fruits[i].setType(FruitType.values()[Math.min(Math.max(type, 0), FruitType.values().length - 1)]);
            fruits[i].setActive(active == 1);
        }
        
        // Actualizar datos del cliente (MODIFICADO)
